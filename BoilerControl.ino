...
void build(gh::Builder& b) {
  ...
  if (b.beginRow()) {
    b.Input(&name).label(F("Name")).size(2);   // вводим имя
    b.Input(&value).label(F("Value")).size(3); // вводим Telegram ID
    b.endRow();
  }  // end if
  ...
  if (b.beginRow()) {
    if (b.Button().label(F("Save")).size(1).click()) {
      dataId.set(name, value);      // записываем пару Имя - Telegram ID
      chatIdChanged = 1;            // поднимаем флаг изменения chatId
    }
    if (b.Button().label(F("Del")).size(1).click()) {
      dataId.remove(name);          // удаляем пару Имя - Telegram ID
      chatIdChanged = 1;            // поднимаем флаг изменения chatId
    }
    b.endRow();
  }  // end if
  
  b.Text_("pairs", dataId);   // выведем содержимое базы данных как текст  
  b.Text_("chatId", chatId);  // выведем содержимое chat_id как текст
...
void setup() {
  ...
  if (data.get("tempBatBorder").toString() == "") data.set("tempBatBorder", 30);  // запись начального значения уставки температуры батареи
  if (data.get("tempOutBorder").toString() == "") data.set("tempOutBorder", 15);  // запись начального значения уставки температуры воздуха
  if (data.get("humCorrection").toString() == "") data.set("humCorrection", 0 );  // запись начального значения humCorrection
  if (data.get("alarmFlag").toString() == "") data.set("alarmFlag", 0);           // запись начального значения отслеживания тревог
...
void loop() {
  ...
  if (((humidity > heat4xBorder) && heat4xTmr.tick()) || ((millis() - heatTmr) > heatPeriod)) {
    ...
    humidity = tempHumidity + (int)data.get("humCorrection");     // SensirionI2cSht4x.h
  ...
  if (sensorReadTmr.tick()) {
    ...
    rssi = WiFi.RSSI();
    humidity = humidity + (int)data.get("humCorrection");
    showScreen();  // вывод показаний датчиков на экран
  ...
  if ((tempBat < (int)data.get("tempBatBorder")) && botAlarmTmr.tick() && data.get("alarmFlag").toBool()) {
    String buf;
    buf = "Т батареи " + String(tempBat) + "\n";
    buf += "ниже заданной " + data.get("tempBatBorder").toString() + " !\n";
  ...
  if ((tempOut < (int)data.get("tempOutBorder")) && botAlarmTmr.tick() && data.get("alarmFlag").toBool()) {
    String buf;
    buf = "Т воздуха " + String(tempOut) + "\n";
    buf += "ниже заданной " + data.get("tempOutBorder").toString() + " !\n";
    bot.sendMessage(buf, chatId);  // отправили сообщение по списку
  }
